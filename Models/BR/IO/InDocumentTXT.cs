...
namespace Applet.Nat.Api.Br.Models
{
    public class InDocumentTXT : IRawDocument
    {
        ...
        {
            List<DocumentUser> lcoDocumentUsers = new List<DocumentUser>();
            ...
            try
            {
                ...
                #endregion
                string[] lcostrRawDocuments = lioCuit.GetEncoding().GetString(Convert.FromBase64String(Format.UnCompress(ivstrRaw ?? string.Empty, lioCuit.GetEncoding()))).Split("XXXINICIO");

                foreach (string livstrRawDocument in lcostrRawDocuments)
                {
                    if (string.IsNullOrEmpty(livstrRawDocument.Trim())) continue;
                    cvstrInDocumentLines = livstrRawDocument.Split("\n");
                    ...
                    lioDocumentUser.ivstrInputData = Format.Compress(Convert.ToBase64String(lioCuit.GetEncoding().GetBytes(livstrRawDocument)));
                    ...
                    foreach (ServiceMapperItem lioMapperItem in lioServiceMapper.coItems)
                    {
                        ...
                        if (lioMapperItem.ivstrCoord.StartsWith("U"))
                        {
                            ...
                            if (lioMapperItem.ivstrProperty.StartsWith("coOpcionales."))  // caso opcionales en una columna
                            {
                                if (lioDocumentUser.coOpcionales == null)
                                    lioDocumentUser.coOpcionales = new List<UxDocumentOpcional>();
                                livstr = lioMapperItem.ivstrProperty.Split('.')[1];
                                if (livstr.StartsWith("ivstrId"))
                                    livstr = livstr.Replace("ivstrId", "");
                                else if (livstr.StartsWith("ivstrValor"))
                                    livstr = livstr.Replace("ivstrValor", "");
                                else
                                {
                                    lioSbErrors.AppendLine($"Linea {livnumLine + 1}: Opcional {Resources.lioE_ObjectNoM}");
                                    livblnOK = false;
                                    continue;
                                }
                                if (!short.TryParse(livstr, out livnro))
                                {
                                    lioSbErrors.AppendLine($"Linea {livnumLine + 1}: Opcional {Resources.lioE_ObjectNoM}");
                                    livblnOK = false;
                                    continue;
                                ...
                                while (true)
                                {
                                    if (lioDocumentUser.coOpcionales.Count < livnro + 1)
                        ...
                        if (lioMapperItem.ivstrCoord.StartsWith("R"))
                        {
                            ...
                            if (lioMapperItem.ivstrProperty.StartsWith("coAsociados"))
                            {
                                ...
                                if (lioMapperItem.ivblnColumnHeader ?? true)
                                    livnumLine++; //saltea linea de titulos
                                while (true)
                                {
                                    livnumLine++; //recorre las lineas hasta el proximo identificador "*"
                            ...
                            #endregion
                            #region Opcionales
                            else if (lioMapperItem.ivstrProperty.StartsWith("coAdicionales"))
                            {
                                ...
                                while (true)
                                {
                                    ...
                                    livblnOK = true;
                                    foreach (ServiceMapperItem lioMapperItemR in lioServiceMapper.coItems.Where(x => x.ivstrProperty.StartsWith("coAdicionales.")))
                                    {
                                        if (!GetCoordinates(lioMapperItemR.ivstrCoord, out livstrIdInicio, out livnumFrom, out livnumOffset, out livnumLen))
                                        {
                                            lioSbErrors.AppendLine($" {lioMapperItemR.ivstrProperty} identificador de propiedad invalido (****XXXX:N,N)");
                    ...
                    lcoDocumentUsers.Add(lioDocumentUser);
                }
                return lcoDocumentUsers.ToArray();
            }
        ...
        public string ToPrint()
        {
            ...
            XmlNode lioXmlNode, lioXmlParent;
            Dictionary<string, XmlNode> lcoXmlNodesToClone;
            ...
            XmlDocument lioXmlPristine = new XmlDocument();
            lioXmlPristine.LoadXml(lioXmlToPrinter.OuterXml);
            ...
            foreach (ServiceMapperItem lioMapperItem in lioServiceMapper.coItems)
            {
                ...
                else if (lioMapperItem.ivstrCoord.StartsWith("R"))
                {
                    ...
                    lcoXmlNodes = null;
                    livstrXPath = null;
                    lcoXmlNodesToClone = new Dictionary<String, XmlNode>();
                    while (true)
                    {
                        livnumLine++;
                        ...
                        foreach (ServiceMapperItemXPath lioServiceMapperItemXPath in lioMapperItem.coXPaths)
                        {
                            if (!string.IsNullOrEmpty(lioServiceMapperItemXPath.ivstrParent))
                            {
                                ...
                                if (!lcoXmlNodesToClone.ContainsKey(lioServiceMapperItemXPath.ivstrParent))
                                {
                                    lcoXmlNodesToClone[lioServiceMapperItemXPath.ivstrParent] = lcoXmlNodes.Item(0); // guarda el primer nodo repetitivo para clonarlo
                                }
                                if (lcoXmlNodes.Count < livnumRepeticion)
                                {
                                    //agrega un nodo nuevo al padre
                                    lioXmlNode = lioXmlToPrinter.ImportNode(lcoXmlNodesToClone[lioServiceMapperItemXPath.ivstrParent], true);
                                    if (lioXmlNode == null)
                                    {
                                        lioSbErrors.AppendLine($"Error en clonado de nodo para nueva linea: {lioMapperItem.ivstrProperty} xpath {lioServiceMapperItemXPath.ivstrParent} {Resources.lioE_ObjectNoM}");
                                        continue;
                                    }
                                    //clona el nodo y lo agrega al padre
                                    lioXmlParent = lcoXmlNodesToClone[lioServiceMapperItemXPath.ivstrParent].ParentNode;  // obtiene el padre del nodo repetitivo
